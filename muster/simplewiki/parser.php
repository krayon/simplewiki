<?php
// Muster Software Copyright (c) Henrik Bechmann, Toronto, Canada 2009-2013. All rights reserved.
...
//		$rules->char =  '(?P<char> ([\\w\\s]+$|. ))'; //faster, but misses raw url's, twice as slow as markup filter
		...
		#================================[ special decorators ]===============================#
		#--------------------------------[ span decoration ORIGINAL]----------------------------------#
#		/// \b span: span decorator '% ... %'. Empty decorator (optional) connotes end of span
#		$rules->span = '(?P<span> %(s\s+(?P<span_decorator>'.$argchars.'+?))?% )';
		#--------------------------------[ span decoration ]----------------------------------#
		/// \b span: span decorator '% ... %'. Empty decorator (optional - %%) connotes end of span
		// added lookbehind for % to allow "%" in decoration string
		$rules->span = '(?P<span> %
			(s
			\s+(?P<span_decorator>'.$argchars.'+?))?
			((?<!~)%))';
		#--------------------------------[ itagdef ]----------------------------------#
		/// \b itag: inline tag '%= ... %'. Empty decorator (optional %=%) connotes end of itag
		$rules->itag = '(?P<itagdef> %=
			((?P<itagdef_selector>[\w][\w-.:]*)
			(\s+(?P<itagdef_decorator>'.$argchars.'+?))?
			\s*)?
			((?<!~)%))';
		#--------------------------------[ vtagdef ]----------------------------------#
		/// \b vtag: void tag '%== ... %'.
		$rules->vtag = '(?P<vtagdef> %==
			((?P<vtagdef_selector>[\w][\w-.:]*)
			(\s+(?P<vtagdef_decorator>'.$argchars.'+?))?
			\s*)
			((?<!~)%))';
		#--------------------------------[ block dividers ]-----------------------------------#
...
// embedded '-' character option for block_selector name [\w][\w-.:]* added dec 23, 2013
// was \w+; this accommodates muster widget block names - mw-widget etc. 
*/
		$rules->blockdef = '
			^(?P<blockdef>
			\n?(?P<block_indent>[\t\x20]*)\(:(?P<block_selector>[\w][\w-.:]*)(\s+(?!:\))(?P<block_decorator>'.$argchars.'+?))? \s* :\)
			\s*?(?P<block_inline>.*) $
			(?P<block_content>(\n.*$)*?)
		...
		#--------------------------------[ vblock declarations ]------------------------------#
/** \b vblockdef: void block declarations
// embedded '-' character option for block_selector name [\w][\w-.:]* added dec 23, 2013
// was \w+; this accommodates muster widget block names - mw-widget etc. 
*/
		$rules->vblockdef = '
			^(?P<vblockdef>
			\n?[\t\x20]*
			\(::(?P<vblock_selector>[\w][\w-.:]*)
			(\s+(?!:\))
			(?P<vblock_decorator>'.$argchars.'+?))? 
			\s* :\)
			\s*?$
		)';
		#--------------------------------[ macros ]--------------------------------#
	...
	protected function _set_re($rules)
	{
		...
		$this->link_inline_re = "/\n"
			...
			array($rules->code, $rules->image, $rules->strong, 
				$rules->emph, $rules->span, $rules->vtag, $rules->itag, $rules->linebreak, 
		...
		$this->inline_re = "/\n" 
			. implode("\n|\n", 
				...
				$rules->span, $rules->vtag, $rules->itag, $rules->linebreak, $rules->escape, $rules->char))
			. "\n/x";
		...
		$this->block_re = "/\n" 
			. implode("\n|\n",
				array($rules->blankline, $rules->vblockdef, $rules->blockdef, $rules->heading, 
...
//		$raw = preg_replace('/\n```.*$/m','',$raw);
		...
		$raw = preg_replace('/(^|\W)((?<!\[\[|{{|~)(http[s]?|mailto):\/\/\S+)/','$1[[$2]]',$raw); // add markup to raw url; // was \S+\w
...
*/
    protected function _char_node($preg_groups) // can create text leaf node
	{
        ...
        if (!isset($this->_leaftextnode->textcontent))
...
*/
	...
	#--------------------------------[ itagdefs ]--------------------------------#
/** Create an inline tag node. SimpleWiki_DocNode::ITAGDEF.
	An itag with or without a decorator opens a new itag. 
	An empty itag (%%) closes an existing itag, or is written as text if no itag has been opened.
	...
	@return void which removes the found string from the raw text
*/
	protected function _itagdef_node($preg_groups)
	{
		# span
		isset($preg_groups['itagdef_decorator']) or ($preg_groups['itagdef_decorator'] = '');
		isset($preg_groups['itagdef_selector']) or ($preg_groups['itagdef_selector'] = '');
		$decorator = $preg_groups['itagdef_decorator'];
		$decorator = str_replace('~','',$decorator); // escape character may be needed for embedded % - hb dec 23, 2013
		$selector = $preg_groups['itagdef_selector'];
		if ($selector) // new itag
		{
            $node = $this->_curnode = new DocNode(DocNode::ITAGDEF, $this->_curnode);
			$this->set_node_decoration($node,$decorator);
			// set selector...
			$node->itag = $selector;
			$node->source_markup = $preg_groups['itagdef'];
			$this->_leaftextnode = NULL;
		}
		elseif ($this->_curnode->type == DocNode::ITAGDEF) // closing existing itag
		{
			$this->_curnode->source_close_markup = $preg_groups['itagdef'];
			if (!empty($this->_curnode->parent))
			{
				$this->_curnode = $this->_curnode->parent;
				$this->_leaftextnode = NULL;
		...
		else // error, return text
		{
			if (is_null($this->_leaftextnode))
				$this->_leaftextnode = new DocNode(DocNode::TEXT, $this->_curnode);
			$this->_leaftextnode->textcontent .= $preg_groups['itagdef'];
		}
	}
	#--------------------------------[ vtagdefs ]--------------------------------#
/** Create an inline tag node. SimpleWiki_DocNode::ITAGDEF.
	...
	@return void which removes the found string from the raw text
*/
	protected function _vtagdef_node($preg_groups)
	{
		# vtag
		isset($preg_groups['vtagdef_decorator']) or ($preg_groups['vtagdef_decorator'] = '');
		isset($preg_groups['vtagdef_selector']) or ($preg_groups['vtagdef_selector'] = '');
		$decorator = $preg_groups['vtagdef_decorator'];
		$decorator = str_replace('~','',$decorator); // escape character may be needed for embedded % - hb dec 23, 2013
		$selector = $preg_groups['vtagdef_selector'];
		if ($selector) // new itag
		{
			$node = new DocNode(DocNode::VTAGDEF, $this->_curnode);
			$this->set_node_decoration($node,$decorator);
			// set selector...
			$node->itag = $selector;
			$node->source_markup = $preg_groups['vtagdef'];
			$this->_leaftextnode = NULL;
		}
		else // error, return text
		{
			if (is_null($this->_leaftextnode))
				$this->_leaftextnode = new DocNode(DocNode::TEXT, $this->_curnode);
			$this->_leaftextnode->textcontent .= $preg_groups['vtagdef'];
		}
	}
	#================================[ special decorators ]=============================#
	#--------------------------------[ span decoration ]--------------------------------#
	...
	protected function _span_node($preg_groups)
	{
		...
		else // error, return text
		{
			if (is_null($this->_leaftextnode))
				$this->_leaftextnode = new DocNode(DocNode::TEXT, $this->_curnode);
            if (!isset($this->_leaftextnode->textcontent))
                $this->_leaftextnode->textcontent = '';
...
/** Create a block node. SimpleWiki_DocNode::BLOCKDEF. Set reference to closest document, blockdef, list_item or def_desc ancestor.
	...
	@return void which removes the found string from the raw text
*/
	protected function _vblockdef_node($preg_groups)
	{
		isset($preg_groups['vblock_selector']) or ($preg_groups['vblock_selector'] = '');
		isset($preg_groups['vblock_decorator']) or ($preg_groups['vblock_decorator'] = '');
		$decorator = $preg_groups['vblock_decorator'];
		$selector = $preg_groups['vblock_selector'];
		if ($selector) // new itag
		{
			$node = new DocNode(DocNode::VBLOCKDEF, $this->_curnode);
			$this->set_node_decoration($node,$decorator);
			// set selector...
			$node->blocktag = $selector;
			$node->source_markup = $preg_groups['vblockdef'];
			$this->_leaftextnode = NULL;
		}
		else // error, return text
		{
			if (is_null($this->_leaftextnode))
				$this->_leaftextnode = new DocNode(DocNode::TEXT, $this->_curnode);
			$this->_leaftextnode->textcontent .= $preg_groups['vblockdef'];
		}
	}
