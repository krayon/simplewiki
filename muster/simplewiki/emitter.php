...
class Emitter
{
	...
	protected $_blocktags = // could be replaced or changed by client
		...
		(
			...
			'figure','figcaption','article','aside','footer','section','header','hgroup', 'nav', 'main', # new html5
			'details', 'dialog', 'summary', 'ruby', 'rt', 'rp', # more html5
			'address', 'map',
			'object','audio','video','canvas', # media
			'form','output','datalist', # forms
			'mw-module','mw-widget','mw-settings','mw-marker', # muster widgets - reserved
			'dlmodule','dlwidget','dlsettings','dlmarker' # dlml (deprecated)
		);
	protected $_vblocktags = // could be replaced or changed by client
		array
		(
			'hr', 'area', 'param','source','track','col'
		);
	/// supported inline tags
	protected $_itags = // could be replaced or changed by client
		array
		(
			'b', 'big', 'i', 'small', 'ins','del',
			'abbr', 'cite', 'code', 'dfn', 'em', 'kbd', 'strong', 'samp', 'var',
			'a', 'bdo', 'img', 'object', 'q', 'span', 'sub', 'sup',
			'button', 'label', 'select', 'option', 'textarea',
			'progress', 'bdi', 'mark', 'meter', 'time' # html5
		);
	/// supported void tags
	protected $_vtags = // could be replaced or changed by client
		array
		(
			'br','wbr', 'link', 'img', 
			 'input',
			'command','embed','keygen'
		);
	/**@}*/
	...
	protected $_vblockdef_handler;
	protected $_itagdef_handler;
	protected $_vtagdef_handler;
	/**@}*/
/**@{ @name Creation */	
	/** The constructor.
	...
	*/
	protected function set_rules()
	{
		...
		$rules->extern = "(?P<external_address>(?P<external_proto>$proto)
			:\/\/(?P<external_selector>\S+))"; // was \S+\w
		$rules->mail = "(?P<external_mailaddress>(?P<external_mailproto>mailto)
			:(?P<external_mailselector>\S+))"; // was \S+\w
		$rules->symlink = '
...
/// get or set collection of block declarations recognized by emitter
	public function vblocktags($taglist = NULL)
	{
		if (!is_null($taglist))
			$this->_vblocktags = $taglist;
		return $this->_vblocktags;
	}
/// get or set collection of block declarations recognized by emitter
	public function itags($taglist = NULL)
	{
		if (!is_null($taglist))
			$this->_itags = $taglist;
		return $this->_itags;
...
/// get or set collection of block declarations recognized by emitter
	public function vtags($taglist = NULL)
	{
		if (!is_null($taglist))
			$this->_vtags = $taglist;
		return $this->_vtags;
	...
	public function register_property_callouts($callouts) 
	{
		...
		foreach ($callouts as $nodetype => $propertycallouts)
		{
			...
			else
			{
				...
				foreach($propertycallouts as $property =>$methodref)
				{
					$calloutlist[$nodetype][$property] = $methodref;
				}
			}
		}
...
*/
	...
	public function register_vblockdef_handler($handler)
	{
		$this->_vblockdef_handler = $handler;
	}
	public function register_itagdef_handler($handler)
	{
		$this->_itagdef_handler = $handler;
	}
	public function register_vtagdef_handler($handler)
	{
		$this->_vtagdef_handler = $handler;
	}
/**@}*/
	#---------------------------[trigger callouts]-------------------------------------#
	...
	*/
	protected function prepare_image_node($node)
	{
		...
		elseif (!empty($node->linkparts->externaladdress)) {
			$node->decoration->attributes['src'] = 
				$node->linkparts->externaladdress;
		}
		# raw address
		else {
		...
		// in any case set the attribute delimiter to double quote
...
/**	emit html for itag.
	...
	@return object modified document node
*/
	protected function _itagdef_emit($node) // i decorator "s"
	{
		$itag = $node->itag;
		$node->knowntag = in_array($itag,$this->_itags);
		if ($node->knowntag) {
			$node->opentag_head = "<". $itag;
			$node->opentag_tail = ">";
			$node->closetag = "</" . $itag . ">";
		} else {
			$node->opentag_head = $node->source_markup;
			$node->opentag_tail = '';
			$node->closetag = isset($node->source_close_markup)?
				$node->source_close_markup:'';
		}
		$node->elementcontent = '';
		if (isset($this->_itagdef_handler)) // handler deals with decoration
			$node = call_user_func($this->_itagdef_handler,$node);
...
/**	emit html for itag.
	...
	@return object modified document node
*/
	protected function _vtagdef_emit($node) // i decorator "s"
	{
		$vtag = $node->itag;
		$node->knowntag = in_array($vtag,$this->_vtags);
		if ($node->knowntag) {
			$node->opentag_head = "<". $node->itag;
			$node->opentag_tail = " />";
		} else {
			$node->opentag_head = $node->source_markup;
			$node->opentag_tail = '';
		}
		if (isset($this->_vtagdef_handler)) {
			$node = call_user_func($this->_vtagdef_handler,$node);
		}
		if ($node->knowntag)
		{
			$node = $this->prepare_node($node);
	...
	#---------------------------[ vblockdef declarations ]--------------------------------#
/**	emit html for vblockdef.
	@param object $node document node
	@return object modified document node
*/
	protected function _vblockdef_emit($node) // i decorator "s"
	{
		$vtag = $node->blocktag;
		$node->knowntag = in_array($vtag,$this->_vblocktags);
		if ($node->knowntag) {
			$node->opentag_head = "\n<". $node->blocktag;
			$node->opentag_tail = " />";
		} else {
		...
		if (isset($this->_vblockdef_handler)) {
			$node = call_user_func($this->_vblockdef_handler,$node);
		}
		if ($node->knowntag)
		{
			$node = $this->prepare_node($node);
